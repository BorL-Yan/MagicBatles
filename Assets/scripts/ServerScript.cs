...
namespace MagicBattles.Server
{
    ...
    public class ServerScript : MonoBehaviour
    {
        ...
        Thread thread;
        [SerializeField]
        private Dictionary<ClientStats, TcpClient> clients = new Dictionary<ClientStats, TcpClient>();
        ...
        private async void ReadClientMessage(byte[] data)
        {
            switch (data[0])
            {
                case ((byte)csm.UseAbility):
                {
                    UseAbility useAbility = new UseAbility();
                    useAbility.Decode(data);
                    AttackEnamy(useAbility);
                            
                    break;
                }
                case ((byte)csm.CanMakeChosenServer):
                {
                    break;
                }
            }
        ...
        private void AttackEnamy(UseAbility useAbility)
        {
